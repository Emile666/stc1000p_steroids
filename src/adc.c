...
uint16_t read_adc(uint8_t ch)
{
    ...
    ADC_CSR      &= ~CH_MASK;       // Clear ADC channel 
    ADC_CSR      |= (ch & CH_MASK); // Select ADC channel
    ADC_TDRL      = 0x0C;           // Disable Schmitt-Trigger of ADC channels 3 and 2
    ADC_CR1      |= ADON;           // Turn ADC on, note a 2nd set is required to start the conversion.
    ADC_CR3      &= ~DBUF;
    ADC_CR2      |= ALIGN;          // Data is right aligned.
    while (!(ADC_CR1 & ADON)) ;     // wait until ADC is turned on

    for (i = 0; i < ADC_AVG; i++)
    {
        ADC_CR1     |= ADON;        // This 2nd write starts the conversion.
        while (!(ADC_CSR & EOC)) ;  // wait until conversion is complete
        ...
        ADC_CSR     &= ~EOC;	      // Reset conversion complete flag
    } // for
    ADC_CR1 &= ~ADON;               // Disable the ADC
