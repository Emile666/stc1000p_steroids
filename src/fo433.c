...
int8_t   fo433_sec_count             = FO433_SEC/2; // second timer for fo433
...
void fo433_isr(void)
{
	...
	{
		case FO433_INIT:
                PB_ODR    &= ~LED1; // Green Led on frontpanel off
				fo433_byte = 0;
				if (fo433_start) 
					fo433_std   = FO433_RDY_SEND;
				break;
		case FO433_RDY_SEND: // ready to send 1 byte
                PB_ODR    |= LED1; // Led on frontpanel on
				fo433_bit  = 0x80;
...
/*-----------------------------------------------------------------------------
  ...
  Variables: temp: the temperature to send out
  Returns  : -
  ---------------------------------------------------------------------------*/
void fo433_fsm(int16_t temp)
{
	...
	if (!fo433_start && (fo433_sec_count < 3))
	{
	   ...
	   di            = ((uint8_t)eeprom_read_config(EEADR_MENU_ITEM(dI))) << 4;
	   t             = temp;
